#!/bin/bash
# OpenClaw Recovery UI - 桌面點擊啟動版
# 雙擊即可開啟恢復介面

# 取得腳本所在目錄
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
RECOVERY_SCRIPT="$SCRIPT_DIR/restore.command"
...
# 建立 AppleScript App 包裝器
APP_DIR="$SCRIPT_DIR/🛠️ 系統恢復.app"
mkdir -p "$APP_DIR/Contents/MacOS"

# 建立啟動腳本
cat > "$APP_DIR/Contents/MacOS/啟動" <<'LAUNCHER_EOF'
#!/bin/bash
SCRIPT_DIR="$(cd "$(dirname "$0")/../.." && pwd)"
RECOVERY_SCRIPT="$SCRIPT_DIR/restore.command"
...
tell application "Terminal"
    activate
    do script "cd \"$(dirname "$RECOVERY_SCRIPT")\" && \"$(basename "$RECOVERY_SCRIPT")\"; exit"
    set window 1's current settings to settings set "Homebrew"
end tell
APPLESCRIPT
LAUNCHER_EOF

chmod +x "$APP_DIR/Contents/MacOS/啟動"
...
<dict>
    ...
    <string>com.openclaw.recovery</string>
    <key>CFBundleName</key>
    <string>🛠️ 系統恢復</string>
