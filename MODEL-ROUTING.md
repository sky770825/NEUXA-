# NEUXA — 模型路由與記憶延續策略 v2.0

> 更新：2026-02-27（修正為實際運行的路由策略）
> 目標：品質優先，成本可控，身份永續
...
[Kimi K2.5] —— 備援指揮（品質接近，自動接手）
    ↓ 失敗/不可用
[Ollama qwen3:8b] —— 本地應急（$0，品質較低但永遠可用）
    ↓ 需要高品質產出
...
老蔡確認：**品質比省錢更重要。** Ollama 8b 模型做指揮會退化。

- Gemini Flash 免費額度足夠日常使用
- Kimi K2.5 是 Gemini 掛了的即時替補
- Ollama 只做子任務（文案生成、簡單分析、格式轉換），不當指揮
...
| **Ollama qwen3:8b** | 本地子任務工人 | $0 | 文案生成、簡單分析、格式轉換 | ✅ 完整 |
| **Ollama deepseek-r1:8b** | 本地推理 | $0 | 邏輯推理、Debug 輔助 | ✅ 完整 |

**關鍵規則：指揮官 = 雲端模型。絕不用 Ollama 當指揮。**
